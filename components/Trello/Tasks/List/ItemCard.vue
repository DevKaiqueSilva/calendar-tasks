<template>
    <v-card class="pa-2" color="white" @click="onDetalhe">
        ...
        <v-layout align-center wrap class="mt-1">
            <v-card v-if="item.dataEntrega!=null" color="green" class="body-2 pa-1 mr-1 white--text">
                <v-layout align-center>
                <v-icon color="white" size="15" class="pr-1">mdi-clock</v-icon>
                {{item.dataEntrega!=null?item.dataEntrega.substring(0,10).split("-").reverse().join("/"):''}}
                </v-layout>
            </v-card>
